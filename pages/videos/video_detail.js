...
client.videos.detail(videoId, data => {
    ...
    const /** {Object} */ hdVideo = video_files.find(item => item.quality === "hd") || video_files[0];
    ...
    video_files.forEach(item => {
        ...
        $downloadMenu.innerHTML += `
          <a href="${link}" download class="menu-item">
            <span class="label-large text">${(quality ?? "Video").toUpperCase()}</span>
    ...
    $detailWrapper.innerHTML = `
        <div class="detail-banner" style="aspect-ratio: ${width} / ${height};">
            <video poster="${image}" controls playsinline class="img-cover" data-video>
